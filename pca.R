...
fit_pc2_glmm <- glmmTMB(
  PC2 ~ TratamientoDesc * StudyPeriodWeek + Work_status +  (1 | ParticipantID),
...
fit_pc3 <- lmer(
  PC3 ~ TratamientoDesc * StudyPeriodWeek + Work_status + (1 | estacion/ParticipantID ),
  data = pca_for_model
)
summary(fit_pc3)
...
model_pc1 <- glmmTMB(
  PC1 ~ Work_status + TratamientoDesc * StudyPeriodWeek + (1 | ParticipantID/estacion),
  data = pca_for_model,
  family = gaussian()
)
summary(model_pc1)
...
plot_model(model_pc1, 
           type = "pred", 
           terms = c("StudyPeriodWeek", "TratamientoDesc", "estacion", "Work_status"),
           ci.lvl = 0.95) +
